    ...
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4" onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md animate-fade-in p-6">
        ...
        <div className="bg-slate-50 rounded-xl p-4 mb-4">
          ...
          <p className="text-sm text-slate-500">By {achievement.userId?.name} ({achievement.userRole || achievement.userId?.role})</p>
        </div>
        {action === 'verify' ? (
          <div className="mb-4">
              ...
              <div className="flex items-start gap-4">
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-slate-500 mb-2">
                    ...
                    {(a.userRole === 'student' || a.userId?.role === 'student') && <span>Year {a.userId?.year} · Sec {a.userId?.section} {a.userId?.rollNumber ? '· ' + a.userId.rollNumber : ''}</span>}
                  </div>
